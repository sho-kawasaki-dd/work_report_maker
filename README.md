...
正常に実行されると、プロジェクトルートに `full_report.pdf` が出力されます。CLI と GUI は共通の `generate_full_report(...)` バックエンドを使い、出力後には pikepdf による構造最適化が適用されます。

PDF 最適化を無効にして CLI 生成したい場合は、次を使います。

```powershell
uv run python -m work_report_maker --no-optimize-pdf
```
...
| Step | ページ               | 内容                                                                             |
| ---- | -------------------- | -------------------------------------------------------------------------------- |
| 1    | ProjectNamePage      | プロジェクト名（必須）と PDF 保存先フォルダ                                      |
...
- 1 ページ目では `Back` は表示されません。
- 2 ページ目以降では、右下の `Next` ボタンの左側に `Back` が表示され、1 ページずつ前のページへ戻れます。
- `Cancel` を押すと「プロジェクトを破棄してアプリを終了しますか？」という確認ダイアログが表示されます。
- 確認ダイアログで「いいえ」を選ぶと入力中の内容を保持したままウィザードへ戻ります。
- 「はい」を選んだ場合でも、PDF 生成中または画像読み込み停止中は既存のガードが優先され、処理中メッセージを表示して終了を保留します。
...
- **PDF 保存先**: Step 1 で指定した保存先フォルダを次回以降の既定値として保存します。未設定時は Desktop を使い、Desktop が見つからない場合はプロジェクトルートへフォールバックします。Finish 時には保存ダイアログが開き、初期ファイル名として `プロジェクト名.pdf` が提示されます。
- **PDF 生成後に閉じる設定**: Step 1 のチェックボックスで、PDF 生成成功後にアプリを自動終了するかを切り替えられます。設定は PDF 保存先と同じ永続設定ファイルへ保存され、次回起動時にも引き継がれます。未チェックの場合は PDF 保存後もウィザードを開いたままにし、入力内容を保持したまま再生成できます。
